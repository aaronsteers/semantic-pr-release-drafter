...
const COMMIT_TYPES = {
  ...
  infra: { title: 'Infrastructure', bump: 'patch' },
}
...
/**
 * Merged pull request associated with a commit that belongs to the target
 * repository. Nodes lacking `baseRepository` (e.g. local-git mocks) qualify;
 * when `repoNameWithOwner` is given, nodes naming a different base repository
 * (e.g. a cross-repo PR) are skipped.
 */
const mergedPullRequestFor = (commit, repoNameWithOwner) =>
  commit.associatedPullRequests?.nodes?.find(
    (p) =>
      p.merged &&
      (!repoNameWithOwner ||
        !p.baseRepository?.nameWithOwner ||
        p.baseRepository.nameWithOwner === repoNameWithOwner)
  )

/**
 * Message to parse for a commit's line item. With `title-source: pr-title`, a
 * merged associated PR's current title replaces the commit subject so a
 * miscategorized or unclear squash subject can be corrected once by retitling
 * the PR. The PR title is the only parseable line; the commit body is kept
 * only for `BREAKING CHANGE:` footers so they still apply. Commits without a
 * merged PR fall back to the commit message.
 */
const messageForParsing = (commit, pr, titleSource) => {
...
class ReleaseChangeLineItems {
  ...
  static fromCommits(
    commits,
    { titleSource = 'commit', repoNameWithOwner } = {}
  ) {
    const items = []
    // With `title-source: pr-title`, a merged PR produces one item set per PR:
    // later commits of the same PR fold into it instead of adding duplicates,
    // and only contribute `BREAKING CHANGE:` footers to the existing items.
    ...
    for (const commit of commits) {
      ...
      if (titleSource === 'pr-title' && pr && seenPrs.has(pr.number)) {
        ...
        core.info(
          `  Commit ${commit.oid || commit.id} folded into PR #${
            pr.number
  ...
  renderWithConfig(config, context = null) {
    ...
    for (const item of this.items) {
      ...
      for (const cat of categorizedItems) {
        ...
        // Check scope matching (case-insensitive on both sides)
        const matchesScope =
          item.scope &&
          commitScopes
            .map((s) => s.toLowerCase())
            .includes(item.scope.toLowerCase())
        ...
        // Determine if item matches this category:
        // - If both scopes and types are specified: AND them (must match both)
        // - If only scopes specified: match scope
        // - If only types specified: match type
        // - If neither specified: no match
        let matches = false
    ...
    // Sort categories by display-order for rendering
    // Categories with display-order are sorted by that value (lower first)
    // Categories without display-order maintain their natural config order after sorted ones
    // Track original indices to ensure stable sort when neither has display-order
    const categoriesWithIndex = categorizedItems.map((category, index) => ({
      category,
      index,
    }))
    const sortedCategories = categoriesWithIndex
      .sort((aEntry, bEntry) => {
        const a = aEntry.category
        const b = bEntry.category
        const aOrder = a['display-order']
        const bOrder = b['display-order']
        ...
        // Neither has display-order: preserve original config order
 ...
 */
const parseCommitsToChangeItems = (
  commits,
  { titleSource = 'commit', repoNameWithOwner } = {}
) => {
  const changeItems = []
 ...
 */
const resolveVersionBumpFromCommits = (commits, config = {}, options = {}) => {
  const changeItems = parseCommitsToChangeItems(commits, options)
