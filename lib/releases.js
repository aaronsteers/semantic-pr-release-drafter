...
const findReleases = async ({
  ...
  if (tagMatcher) {
    ...
    filteredReleases = commitishFilteredReleases.filter((r) =>
  ...
  return { draftRelease, lastRelease, releases }
}
...
const generateReleaseInfo = ({
  ...
  // On a release track a `release-track-template` replaces `template`, so RC
  // drafts can render GA-relative notes while GA drafts keep `template`.
  const bodyTemplate =
    isReleaseTrack && config['release-track-template']
      ? config['release-track-template']
      : config.template

  // GA-relative changelog source. When no separate GA baseline was computed
  // (non-release-track runs, or a first RC whose baseline is already the GA),
  // fall back to the primary commit range so `$CHANGES_SINCE_GA`/`$PREVIOUS_GA_TAG`
  // mirror `$CHANGES`/`$PREVIOUS_TAG`.
  const gaLastRelease = gaBaseline?.lastRelease ?? lastRelease
  const gaCommits = gaBaseline?.commits ?? commits
  const gaMergedPullRequests =
    gaBaseline?.mergedPullRequests ?? mergedPullRequests
  ...
  body = template(
    body,
    {
      ...
      // Only generate the GA-relative changelog when the template uses it —
      // generation walks every commit and logs per item.
      ...(body.includes('$CHANGES_SINCE_GA') && {
  ...
  const versionKeyIncrement = resolveVersionKeyIncrement(
    ...
    isPreRelease,
    lastRelease,
    Boolean(floorVersion) &&
      isPreRelease &&
      Boolean(config['prerelease-identifier'])
  )
  ...
  const versionInfo = getVersionInfo(
    ...
    // overrideVersion: explicit user input (always wins)
