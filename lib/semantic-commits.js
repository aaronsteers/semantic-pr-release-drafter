  ...
  renderWithConfig(config, context = null) {
    ...
    const renderItem = (item) => {
      ...
      return template(changeTemplate, {
        $TITLE: escapeTitle(item.description),
        $NUMBER: prNumber,
    ...
    for (const category of categorizedItems) {
      ...
      // Handle collapse-after: show first N items, collapse the rest
      const collapseAfter = category['collapse-after'] || 0
      const shouldCollapse =
        collapseAfter > 0 && category.items.length > collapseAfter

      if (shouldCollapse) {
        // Render visible items (first N)
        const visibleItems = category.items
          .slice(0, collapseAfter)
          .map((item) => renderItem(item))
          .join('\n')
        changeLog.push(visibleItems)

        // Render collapsed items in <details> block
        const collapsedItems = category.items
          .slice(collapseAfter)
          .map((item) => renderItem(item))
          .join('\n')
        const collapsedCount = category.items.length - collapseAfter
        const summaryText =
          collapsedCount === 1
            ? '1 more change'
            : `${collapsedCount} more changes`
        changeLog.push(
