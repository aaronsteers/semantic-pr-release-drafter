    ...
    for (const commit of commits) {
      ...
      for (const parsed of parsedResults) {
        const item = new ReleaseChangeLineItem({
          ...
          raw: parsed.raw,
          commitSha: commit.oid || commit.id,
          prNumber: pr?.number,
          author,
        })
        items.push(item)
  ...
  renderWithConfig(config, context = null) {
    ...
    for (const item of this.items) {
      ...
      for (const cat of categorizedItems) {
        ...
        if (commitTypes.includes(item.type)) {
          cat.items.push(item)
          found = true
          break
        }
...
const parseSemanticCommit = (message) => {
  ...
  for (const line of lines) {
    ...
    // Strip trailing PR number pattern (e.g., "(#123)") from description
    // since it will be added via the change-template
    const cleanDescription = description.trim().replace(/\s*\(#\d+\)\s*$/, '')
