...
const findCommitsFromLocalGit = ({ localGitRoot, baseRef, context }) => {
  ...
  // Create mock pull requests from commits for changelog generation
  // The changelog generation uses mergedPullRequests, so we need to create
  // mock PR objects from commits in local git mode
  const pullRequests = commits.map((commit) => {
    // Extract PR number from commit message if present (e.g., "feat: add feature (#123)")
    const prMatch = commit.message.match(/\(#(\d+)\)\s*$/)
    const prNumber = prMatch ? Number.parseInt(prMatch[1], 10) : null

    // Remove the PR reference from the title if present
    const title = prMatch
      ? commit.message.replace(/\s*\(#\d+\)\s*$/, '')
      : commit.message
    ...
    return {
      ...
      author: commit.author.user
        ? { login: commit.author.user.login, url: '', __typename: 'User' }
        : { login: commit.author.name, url: '', __typename: 'User' },
      commitSha: commit.id,
    }
  })
