...
| Repository                                                                                            | Prerelease Workflow                                                                                                           | Publish Workflow                                                                                                  | Recent Releases                                                                 |
| ----------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| [semantic-pr-release-drafter](https://github.com/aaronsteers/semantic-pr-release-drafter) (this repo) | [release-draft.yml](https://github.com/aaronsteers/semantic-pr-release-drafter/blob/main/.github/workflows/release-draft.yml) | [release.yml](https://github.com/aaronsteers/semantic-pr-release-drafter/blob/main/.github/workflows/release.yml) | [Releases](https://github.com/aaronsteers/semantic-pr-release-drafter/releases) |
...
Find more examples via [code search](https://github.com/search?q=uses%3A+aaronsteers%2Fsemantic-pr-release-drafter%40&type=code) (requires GitHub sign-in) or by browsing [dependent repositories](https://github.com/aaronsteers/semantic-pr-release-drafter/network/dependents).
...
| `title-source`             | Optional | Source of each changelog line item's title: `pr-title` parses the merged PR's current title, `commit` parses the commit subject. Falls back to the commit subject when no merged pull request is associated with the commit or its title is unavailable. Default: `"pr-title"` |
...
| `latest`                   | Optional | Controls GitHub's "Set as latest release" option for created or updated releases. Can be one of: `true`, `false`, `legacy`. Default `true`; use `false` for package-scoped drafts that should not become the repository's latest release when published.                       |
...
By default (`title-source: pr-title`), line items are parsed from the merged PR's _current_ title, so a miscategorized or unclear squash-commit subject can be corrected durably by editing the PR title — handy for long-lived release drafts that regenerate on every push. Set `title-source: commit` to parse only the commit subject. In both modes the commit body is still scanned for `BREAKING CHANGE:` footers, and commits with no merged PR use the commit message. In `pr-title` mode each merged PR yields a single entry even when it landed as multiple commits (merge or rebase merges).
