...
const releaseBranchRule = Joi.object({
  'branch-prefix': Joi.string().min(1),
  'branch-pattern': Joi.string()
    .pattern(/\(\?<version>/)
...
const prereleaseBranchRulesSchema = Joi.array().items(releaseBranchRule)
...
const schema = (context) => {
  ...
  return Joi.object()
    .keys({
      ...
      'title-source': Joi.string()
        .valid('pr-title', 'commit')
        .default(DEFAULT_CONFIG['title-source']),
      ...
      'prerelease-branch-rules': Joi.array()
        .items(releaseBranchRule)
        .default(DEFAULT_CONFIG['prerelease-branch-rules']),
...
exports.releaseBranchRule = releaseBranchRule
exports.prereleaseBranchRulesSchema = prereleaseBranchRulesSchema
