 ...
 * @param {string[]} [options.includePaths] - Optional paths used to filter commits
...
const findCommitsFromLocalGit = ({
  ...
  const commits = lines.map((line) => {
    ...
    // The mock PR title keeps the semantic subject (minus the PR-number
    // suffix): with `title-source: pr-title` it replaces the commit subject
    // when the line item is built, so it must reparse identically.
  ...
  const filteredCommits =
    includePathSet.size > 0
      ? commits.filter((commit) =>
          commit.changedPaths.some((changedPath) =>
            pathMatchesIncludePath(changedPath, includePathSet)
          )
        )
      : commits
...
const getChangedPaths = (localGitRoot, commitId) => {
  try {
...
const pathMatchesIncludePath = (changedPath, includePathSet) => {
  ...
  for (const includePath of includePathSet) {
    ...
    if (
      normalizedChangedPath === normalizedIncludePath ||
      normalizedChangedPath.startsWith(`${normalizedIncludePath}/`)
    ) {
      return true
    }
  }
...
const normalizePath = (value) => path.normalize(value).split(path.sep).join('/')
